...
var FileStore = (function () {
    ...
    function _loadDefaultSaveData(extDataDir) {
        if (_hasCepFs()) {
            var pathsToTry = [];
            if (extDataDir) {
                pathsToTry.push(extDataDir + '/sharktools_save.json');
                pathsToTry.push(extDataDir + '/fishtools_save.json');
            }
            if (window.csInterface && typeof SystemPath !== 'undefined') {
                try {
                    var extPath = window.csInterface.getSystemPath(SystemPath.EXTENSION);
                    if (extPath) {
                        pathsToTry.push(extPath.replace(/\\/g, '/') + '/data/sharktools_save.json');
                        pathsToTry.push(extPath.replace(/\\/g, '/') + '/data/fishtools_save.json');
                    }
                } catch (e) {}
            }
        ...
        try {
            ...
            xhr.open('GET', '../data/sharktools_save.json', false);
            xhr.send(null);
            ...
            try {
                ...
                fallbackXhr.open('GET', '../data/fishtools_save.json', false);
                fallbackXhr.send(null);
                if ((fallbackXhr.status === 200 || fallbackXhr.status === 0) && fallbackXhr.responseText) {
                    var parsedFallback = JSON.parse(fallbackXhr.responseText);
                    if (parsedFallback && typeof parsedFallback === 'object') {
                        _cache = parsedFallback;
                        save();
                        return true;
                    }
                }
    ...
    function _loadFromLocalStorage() {
        try {
            var raw = localStorage.getItem('sharkToolsFileStore') || localStorage.getItem('fishToolsFileStore');
            if (raw) {
