(function(window) {
  ...
  reg.register({
    ...
    render(ctx, el, layer, bounds, fx) {
      ...
      // Fast Path: WebGL Hardware RGB Split via SharkToolEngine (< 0.2ms)
      if (typeof window !== 'undefined' && window.SharkToolEngine && window.SharkToolEngine.isReady && typeof window.SharkToolEngine.renderRGBSplit === 'function') {
        if (window.SharkToolEngine.renderRGBSplit(ctx, el, bounds, fx)) {
          return;
        }
      }
