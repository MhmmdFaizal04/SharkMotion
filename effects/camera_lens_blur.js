(function(window) {
  ...
  reg.register({
    ...
    render(ctx, el, layer, bounds, fx) {
      ...
      } else if (typeof window !== 'undefined' && window.SharkEffects && typeof window.SharkEffects.drawBlurred === 'function') {
        ctx.translate(x, y);
        window.SharkEffects.drawBlurred(ctx, el, w, h, r);
        if (bloom > 0) {
          ctx.globalAlpha = bloom * 0.7;
          ctx.globalCompositeOperation = 'screen';
          window.SharkEffects.drawBlurred(ctx, el, w, h, r * 1.4);
        }
      } else {
        try { ctx.drawImage(el, x, y, w, h); } catch (_) {}
      }
