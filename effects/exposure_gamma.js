(function(window) {
  ...
  function initEGGL() {
    ...
    try {
      ...
      const fsSource = [
        ...
        '  float expVal = abs(u_exposure) > 20.0 ? (u_exposure / 50.0) : u_exposure;',
        '  float br = max(0.0, pow(2.0, expVal) + (u_offset / 100.0));',
        '  float gam = 1.0 + (u_gamma / 100.0);',
        '  rgb = rgb * br;',
        '  rgb = (rgb - 0.5) * gam + 0.5;',
  ...
  reg.register({
    ...
    render(ctx, el, layer, bounds, fx) {
      ...
      const exposure = fx && fx.exposure !== undefined ? Number(fx.exposure) : 0;
      const gamma = fx && fx.gamma !== undefined ? Number(fx.gamma) : 0;
      const offset = fx && fx.offset !== undefined ? Number(fx.offset) : 0;

      // Identity pass
      if (Math.abs(exposure) < 0.001 && Math.abs(gamma) < 0.001 && Math.abs(offset) < 0.001) {
        try { ctx.drawImage(el, x, y, w, h); } catch (_) {}
        return;
      }

      // Fast GPU WebGL Pass
      if (!_glFailed && initEGGL()) {
        try {
          const gl = _gl;
          const prog = _glProg;
          const u = _glUniforms;
      ...
      if (typeof window !== 'undefined' && window.SharkEffects && typeof window.SharkEffects.isCanvasFilterSupported === 'function' && window.SharkEffects.isCanvasFilterSupported()) {
        ctx.save();
        let exp = exposure;
        if (Math.abs(exp) > 20) exp = exp / 50;
        const off = offset / 100;
        const gam = 1 + gamma / 100;
        const br = Math.max(0, Math.pow(2, exp) + off);
        ctx.filter = `brightness(${br.toFixed(3)}) contrast(${gam.toFixed(3)})`;
        try { ctx.drawImage(el, x, y, w, h); } catch (_) {}
        ctx.restore();
        return;
      }
