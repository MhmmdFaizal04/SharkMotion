(function(window) {
  ...
  function initSVGL() {
    ...
    try {
      ...
      const fsSource = [
        ...
        '  float satVal = max(0.0, 1.0 + (u_saturation / 100.0) + (u_vibrance / 150.0));',
        '  rgb = mix(vec3(luma), rgb, satVal);',
  ...
  reg.register({
    ...
    render(ctx, el, layer, bounds, fx) {
      ...
      const saturation = fx && fx.saturation !== undefined ? Number(fx.saturation) : 0;
      const vibrance = fx && fx.vibrance !== undefined ? Number(fx.vibrance) : 0;

      // Identity pass
      if (Math.abs(saturation) < 0.001 && Math.abs(vibrance) < 0.001) {
        try { ctx.drawImage(el, x, y, w, h); } catch (_) {}
        return;
      }

      // Fast GPU WebGL Pass
      if (!_glFailed && initSVGL()) {
        try {
          const gl = _gl;
          const prog = _glProg;
          const u = _glUniforms;
          ...
          if (uploaded) {
            ...
            gl.uniform1f(u.vibrance, vibrance);
      ...
      if (typeof window !== 'undefined' && window.SharkEffects && typeof window.SharkEffects.isCanvasFilterSupported === 'function' && window.SharkEffects.isCanvasFilterSupported()) {
        ...
        const vib = vibrance / 150;
        const val = Math.max(0, 1 + sat + vib);
        ctx.filter = `saturate(${val.toFixed(3)})`;
        try { ctx.drawImage(el, x, y, w, h); } catch (_) {}
        ctx.restore();
        return;
      }
