/**
 * SharkTool Studio - Modular Template & Preset Editor Engine
...
(function() {
  ...
  const TemplateEditor = {
    ...
    async _collectReplaceableSlots() {
      ...
      if (projectId && window.SharkDatabase && typeof window.SharkDatabase.getProjectMedia === 'function') {
        try {
          projectMedias = (await window.SharkDatabase.getProjectMedia(projectId)) || [];
        } catch (_) {
          projectMedias = [];
        }
      }
      if (window.SharkDatabase && typeof window.SharkDatabase.getAllMedia === 'function' && projectMedias.length === 0) {
        try {
          const allMedias = (await window.SharkDatabase.getAllMedia()) || [];
          allMedias.forEach(m => {
    ...
    async _handleFileReplacement(file) {
        ...
        if (window.SharkAudioEngine) {
          try {
            ...
            if (oldEntry && oldEntry.el) {
              window.SharkAudioEngine.detachMediaElement(oldEntry.el);
      ...
      if (window.SharkDatabase && typeof window.SharkDatabase.saveMedia === 'function') {
        try {
          await window.SharkDatabase.saveMedia(mediaItemToSave);
        } catch (saveErr) {
          console.warn('[TemplateEditor] SharkDatabase.saveMedia error:', saveErr);
        }
      }
      ...
      if (window.SharkDatabase && state.id) {
        try {
          await window.SharkDatabase.saveProject(state);
        } catch (_) {}
      }
    ...
    seek(timeSec) {
      ...
      if (window.SharkAudioEngine && !this.isPlaying) {
        const pps = window.currentPixelsPerSecond || 80;
        const layers = window.currentProjectState ? (window.currentProjectState.layers || []) : [];
        window.SharkAudioEngine.syncPlayback(layers, this.currentTime, pps);
        window.SharkAudioEngine.pauseAll();
      }
      this.renderFrame();
    },
    ...
    play() {
      ...
      if (window.SharkAudioEngine) {
        if (!window.SharkAudioEngine.isUnlocked) {
          window.SharkAudioEngine.unlock();
        }
      }
      ...
      const loop = (now) => {
        ...
        try {
          ...
          this._playTickCount++;
          if (window.SharkAudioEngine && (this._playTickCount === 1 || this._playTickCount % 4 === 0)) {
            const pps = window.currentPixelsPerSecond || 80;
            const layers = window.currentProjectState ? (window.currentProjectState.layers || []) : [];
            window.SharkAudioEngine.syncPlayback(layers, this.currentTime, pps);
          }
  ...
  window.SharkTemplateEditor = TemplateEditor;
