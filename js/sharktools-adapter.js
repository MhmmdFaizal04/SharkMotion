/**
 * SharkMotion Web App Bridge & Runtime Adapter
 * Fetches original tools/SharkMotion/client/index.html dynamically and:
 * - Converts all asset/script URLs to absolute paths (/tools/SharkMotion/client/...)
 ...
 * 
 * ZERO modifications to tools/SharkMotion/ source repository files.
 */

/**
 * SharkTools Bridge for SharkMotion Web App Integration
 * Handles Anchor Point (grid & center in comp), Cut (front, mid, back), and Align (left, center, right, top, bottom)
 */
window.SharkToolsBridge = (function () {
  ...
  function setAnchorPoint(pos) {
    ...
    selected.forEach(layer => {
      ...
      if (typeof window.recordLayerPropertyChange === 'function') {
        window.recordLayerPropertyChange(layer, 'move');
      }
  ...
  function centerInComp() {
    ...
    const [baseW, baseH] = getResolutionDims();
    const engine = (typeof window !== 'undefined' && (window.SharkToolEngine || window.LayerTransform));
  ...
  function executeTool(toolName, ...args) {
    const directExec = (typeof window.executeSharkTool === 'function')
      ? window.executeSharkTool
      : (window.parent && typeof window.parent.executeSharkTool === 'function' ? window.parent.executeSharkTool : null);
    if (directExec) {
    ...
    if (toolName === 'CUBE' || (typeof toolName === 'string' && toolName.indexOf('GEN_3D') === 0)) {
      if (typeof window.executeSharkTool === 'function') {
        return window.executeSharkTool(toolName, ...args);
      }
    }
    ...
    function mirrorLayers(alter = false) {
      ...
      selected.forEach(layer => {
        ...
        if (typeof window.recordLayerPropertyChange === 'function') {
          window.recordLayerPropertyChange(layer, 'scale');
        }
        if (typeof window.invalidatePreviewCacheForLayer === 'function') {
          window.invalidatePreviewCacheForLayer(layer);
        }
    ...
    function applyBeatKeyframes(type) {
      ...
      const fnExec = (typeof window.executeSharkTool === 'function')
        ? window.executeSharkTool
        : (window.parent && typeof window.parent.executeSharkTool === 'function' ? window.parent.executeSharkTool : null);
    ...
    if (toolName === 'GHST' || toolName === 'WARP' || toolName === 'FISHEYE' || toolName === 'MIDWAVE' || toolName === 'HUESPIN' || toolName === 'EXPO' || toolName === 'FLASH' || toolName === 'LENS' ||
      ...
      const exec = (typeof window.executeSharkTool === 'function')
        ? window.executeSharkTool
        : (window.parent && typeof window.parent.executeSharkTool === 'function' ? window.parent.executeSharkTool : null);
...
window.SharkToolsAdapter = (function () {
  ...
  async function getAdaptedHtml() {
    if (!cachedTemplate) {
      let rawHtml = '';
      try {
        let localRes = await fetch('Extension/extension.html');
        if (!localRes.ok) {
          localRes = await fetch('Extension/index.html');
        }
        if (localRes.ok) {
          const txt = await localRes.text();
          if (txt && !txt.includes('404: Not Found') && !txt.includes('<title>404 Not Found</title>')) {
            rawHtml = txt;
          }
        }
      } catch (localErr) {
        console.warn('Local Extension fetch failed, falling back to direct iframe navigation:', localErr);
      }

      if (!rawHtml) {
        throw new Error('Extension HTML not available via fetch (direct iframe navigation will be used)');
      }
      ...
      // 7. Convert all Relative URLs in <link>, <script>, <img> to Extension/ URLs
      const CLIENT_BASE = 'Extension/';
      ...
      // 8. Enforce SharkMotion Branding on Splash screen
      const splashLogo = doc.querySelector('.splash-logo');
      if (splashLogo) {
        splashLogo.setAttribute('src', 'Extension/assets/Logo.svg');
        splashLogo.setAttribute('alt', 'SharkMotion Tools Logo');
      }
      const splashText = doc.querySelector('.splash-text');
      if (splashText) {
        splashText.textContent = 'SHARKMOTION TOOLS';
      }

      // 9. Inject Web Bridge Polyfill and Web Styles into <head>
      const bridgeScript = doc.createElement('script');
      bridgeScript.textContent = `
        // Extension Version override
        window.EXTENSION_VERSION = 'Latest';

        // Enforce SharkMotion Branding in iframe DOM
        (function enforceSharkBranding() {
          function applyBranding() {
            var sLogo = document.querySelector('.splash-logo');
            if (sLogo) {
              sLogo.src = 'Extension/assets/Logo.svg';
              sLogo.alt = 'SharkMotion Tools Logo';
            }
            var sText = document.querySelector('.splash-text');
            if (sText) {
              sText.textContent = 'SHARKMOTION TOOLS';
            }
            document.title = 'SharkMotion Tools';
          }
          applyBranding();
        ...
        window.FileStore = (function () {
          var _cache = {};
          function getDB() {
            return (window.parent && window.parent.SharkDatabase) || window.SharkDatabase;
          }
          function load() {
            try {
              var db = getDB();
              var raw = db ? db.getSyncSettings() : (localStorage.getItem('sharktools_save') || localStorage.getItem('sharkToolsFileStore'));
          ...
          function save() {
            try {
              ...
              if (db) {
                ...
                localStorage.setItem('sharkToolsFileStore', jsonStr);
              }
            } catch (e) {
        ...
        // Expose SharkTools execution bridge to iframe
        window.SharkTools = {
          executeTool: function (toolName) {
            var args = Array.prototype.slice.call(arguments, 1);
            var parentExec = (window.parent && typeof window.parent.executeSharkTool === 'function')
              ? window.parent.executeSharkTool
              : (typeof window.executeSharkTool === 'function' ? window.executeSharkTool : null);
            if (parentExec) {
              var r = parentExec.apply(window.parent || window, [toolName].concat(args));
              if (r !== undefined) return r;
            }
            var bridge = (window.parent && window.parent.SharkToolsBridge) || window.SharkToolsBridge;
        ...
        window.FishTools = window.SharkTools;
        ...
        window.cep = {
          fs: {
            readFile: function (path) {
              ...
              var db = (window.parent && window.parent.SharkDatabase) || window.SharkDatabase;
              var raw = db ? db.getSyncSettings() : (localStorage.getItem('sharktools_save') || localStorage.getItem('sharkToolsFileStore'));
              var dataObj = {};
            ...
            writeFile: function (path, data) {
              ...
              if (db) {
                ...
                try {
                  ...
                  localStorage.setItem('sharkToolsFileStore', data);
                } catch (e) {
      ...
      styleEl.textContent = `
        ...
        :root, [data-theme="dark"], [data-theme="matcha"], [data-theme="oceanic"] {
          --bg: #050a12 !important;
          --surface: #0a1220 !important;
          --surface2: #0f1d33 !important;
          --border: #162b4c !important;
          --border2: #223e6b !important;
          --accent: #00d2ff !important;
          --accent-h: #38bdf8 !important;
          --accent-rgb: 0, 210, 255 !important;
          --accent-fg: #050a12 !important;
          --text: #e2f1fd !important;
          --text-dim: #7fa5c7 !important;
          --text-mut: #3a567a !important;
        }
        html, body {
          background-color: var(--bg, var(--bg, #050a12)) !important;
          background: var(--bg, var(--bg, #050a12)) !important;
          color: var(--accent, var(--accent, #00d2ff)) !important;
          width: 100%;
        ...
        .bottom-nav, nav, .nav-bar {
          background-color: var(--surface, var(--surface, #0a1220)) !important;
          background: var(--surface, var(--surface, #0a1220)) !important;
          border-top-color: var(--border, var(--border, #162b4c)) !important;
          height: 42px !important;
        }
        .tab-btn {
          color: var(--text-mut, var(--text-mut, #7fa5c7)) !important;
        }
        .tab-btn .tab-label,
        ...
        .style-simple .tab-btn.active {
          background-color: var(--accent, var(--accent, #00d2ff)) !important;
        ...
        .card {
          background-color: var(--surface, var(--surface, #0a1220)) !important;
          background: var(--surface, var(--surface, #0a1220)) !important;
          border-color: var(--border, var(--border, #162b4c)) !important;
        ...
        .style-material-you .tool-btn {
          color: var(--accent, var(--accent, #00d2ff)) !important;
          border-color: var(--border, var(--border, #162b4c)) !important;
          background-color: var(--surface2, var(--surface2, #0f1d33));
        ...
        .style-material-you .tool-btn:hover {
          background-color: var(--accent, var(--accent, #00d2ff)) !important;
          color: #000000 !important;
          border-color: var(--accent, var(--accent, #00d2ff)) !important;
        }
        ...
        .anchor-cell {
          border-color: var(--border, var(--border, #162b4c)) !important;
          background-color: var(--surface2, var(--surface2, #0f1d33)) !important;
        }
        .anchor-cell.active,
        .anchor-cell.anchor-active {
          background-color: var(--accent, var(--accent, #00d2ff)) !important;
          border-color: var(--accent, var(--accent, #00d2ff)) !important;
          box-shadow: 0 0 12px rgba(0, 210, 255, 0.45) !important;
        }
        .status-box {
          background-color: var(--surface2, var(--surface2, #0f1d33)) !important;
          border-color: var(--border, var(--border, #162b4c)) !important;
        ...
        .custom-modal {
          background-color: var(--surface, var(--surface, #0a1220)) !important;
          border: 1px solid var(--border, var(--border, #162b4c)) !important;
          border-radius: 10px !important;
          color: var(--accent, var(--accent, #00d2ff)) !important;
        ...
        .custom-modal-footer .primary-btn {
          background: var(--accent, var(--accent, #00d2ff)) !important;
          color: #000000 !important;
          border: 1px solid var(--accent, var(--accent, #00d2ff)) !important;
        }
        .custom-modal-footer .secondary-btn {
          background: var(--surface2, var(--surface2, #0f1d33)) !important;
          color: var(--accent, var(--accent, #00d2ff)) !important;
          border: 1px solid var(--border, var(--border, #162b4c)) !important;
        }
      `;
  ...
  async function loadIntoIframe(iframeEl, loaderEl) {
    ...
    try {
      ...
      console.warn('SharkToolsAdapter getAdaptedHtml failed, falling back to direct iframe src navigation:', err);
      iframeEl.src = 'Extension/extension.html';
    }
...
window.FishToolsBridge = window.SharkToolsBridge;
window.FishToolsAdapter = window.SharkToolsAdapter;
window.FishTools = window.SharkTools;
