/**
 * SharkMotion Studio - Database Persistence Layer (SharkDatabase)
...
window.SharkDatabase = (function () {
  var DB_NAME = 'SharkStudioDB';
  var DB_VERSION = 3;
  var SETTINGS_KEY = 'sharktools_save';
  var SETTINGS_KEY_ALT = 'sharkToolsFileStore';
  var PROJECTS_KEY = 'sharktools_projects';
  var MEDIA_KEY = 'sharktools_media';
  var DELETED_IDS_KEY = 'sharktools_deleted_ids';
  ...
  async function cleanupLegacyData() {
    if (typeof window !== 'undefined' && window.indexedDB && typeof window.indexedDB.deleteDatabase === 'function') {
      var legacyDbs = [
        'SharkTool_Studio_DB',
        'SharkTool_MediaStorage_DB',
        'SharkTool_Projects_DB',
        'sharkTool_media_db',
        'sharkTool_projects_db'
      ];
  ...
  async function saveSettings(settingsObj) {
    ...
    window.dispatchEvent(new CustomEvent('shark-db-settings-updated', { detail: settingsObj }));
  }
      ...
      await new Promise(function (resolve) {
        ...
        try {
          ...
          tx.oncomplete = function () {
            clearTimeout(safetyTimer);
            window.dispatchEvent(new CustomEvent('shark-db-projects-updated', { detail: { action: 'save', project: project } }));
            resolve(project);
          };
   ...
   * including project JSON metadata, media blobs in SharkStudioDB,
   * and extracted video frame caches in SharkFrameCacheDB.
   * @param {string} projectId
  ...
  async function getProjectTotalSize(projectId) {
    ...
    // 2. Media items footprint in SharkStudioDB (avoiding unnecessary blob URL hydration)
    var mediaList = [];
    try {
    ...
    // 3. Extracted frame cache footprint in SharkFrameCacheDB
    var cacheBytes = 0;
    if (sourceKeys.size > 0 && typeof indexedDB !== 'undefined') {
      ...
      if (typeof indexedDB.databases === 'function') {
        try {
          ...
          var hasFrameDb = dbs && dbs.some(function (d) { return d && d.name === 'SharkFrameCacheDB'; });
      ...
      if (shouldCheckCache) {
        cacheBytes = await new Promise(function (resolve) {
          try {
            var req = indexedDB.open('SharkFrameCacheDB');
  ...
  /**
   * Purges frame cache records belonging to specific source keys from SharkFrameCacheDB
    ...
    if (db && db.objectStoreNames.contains('media')) {
      return new Promise(function (resolve) {
        ...
        var saveTimer = setTimeout(function () {
          if (isDone) return;
          isDone = true;
          console.warn('[SharkDatabase] saveMedia transaction timeout, resolving optimistic');
        ...
        try {
          ...
          tx.onerror = function (err) {
            console.warn('[SharkDatabase] saveMedia tx error:', err);
            saveToLocalFull(mediaItem);
            finish(mediaItem);
          };
          tx.onabort = function (err) {
            console.warn('[SharkDatabase] saveMedia tx aborted:', err);
            saveToLocalFull(mediaItem);
            finish(mediaItem);
          };
        } catch (e) {
          console.warn('[SharkDatabase] saveMedia store.put error:', e);
          saveToLocalFull(mediaItem);
          finish(mediaItem);
        }
      });
    ...
    if (db && db.objectStoreNames.contains('media')) {
      return new Promise(function (resolve) {
        ...
        var timer = setTimeout(function () {
          if (isDone) return;
          isDone = true;
          console.warn('[SharkDatabase] getProjectMedia transaction timeout, returning fallback');
  ...
  async function deleteProject(id) {
    ...
    try {
      window.dispatchEvent(new CustomEvent('shark-db-projects-updated', { detail: { action: 'delete', id: id } }));
    } catch (_) {}
  ...
  async function clearProjects() {
    ...
    window.dispatchEvent(new CustomEvent('shark-db-projects-updated', { detail: { action: 'clear' } }));
    return true;
  }
  ...
  /**
   * Imports a project from an Alight Motion XML file or XML string
   * @param {File|string} fileOrText
   * @param {string} [fileName]
   * @returns {Promise<Object>} The saved project record
   */
  async function importAlightMotionXML(fileOrText, fileName) {
    if (!window.AlightMotionParser || typeof window.AlightMotionParser.parseXML !== 'function') {
      throw new Error("AlightMotionParser not loaded");
    }

    var xmlText = '';
    var name = fileName || 'AlightMotion_Project';

    if (typeof fileOrText === 'string') {
      xmlText = fileOrText;
    } else if (fileOrText && typeof fileOrText.text === 'function') {
      xmlText = await fileOrText.text();
      name = fileOrText.name || name;
    } else if (fileOrText && typeof FileReader !== 'undefined') {
      xmlText = await new Promise(function(resolve, reject) {
        var r = new FileReader();
        r.onload = function(e) { resolve(e.target.result); };
        r.onerror = function() { reject(new Error("Failed to read XML file")); };
        r.readAsText(fileOrText);
      });
      name = fileOrText.name || name;
    }

    var projectData = window.AlightMotionParser.parseXML(xmlText, name);
    if (!projectData) throw new Error("Failed to parse Alight Motion XML");
    ...
    // Save project to IndexedDB / localStorage
    var savedProject = await saveProject(projectData);

    // If media items are declared, store placeholders
    if (Array.isArray(projectData.media) && projectData.media.length > 0) {
      for (var i = 0; i < projectData.media.length; i++) {
        var m = projectData.media[i];
        if (!m) continue;
        var mediaItem = {
          id: m.id || ('med_' + Date.now() + '_' + i),
          projectId: savedProject.id,
          name: m.name || m.filename || ('Media ' + (i + 1)),
          type: m.type || 'image',
          mimeType: m.mimeType || '',
          size: m.size || 0,
          width: m.width || null,
  ...
  async function getCustomEasingPresets() {
    try {
      ...
      const raw = localStorage.getItem('sharktool_custom_easing_presets');
  ...
  async function saveCustomEasingPresets(list) {
    ...
    try {
      localStorage.setItem('sharktool_custom_easing_presets', JSON.stringify(list));
      const s = await getSettings();
  ...
  /**
   * Purges extracted video frames from SharkFrameCacheDB.
  ...
  async function cleanupAllStudioCaches(triggerReason) {
    ...
    console.info('[SharkStudio] 🧹 Purging all non-essential caches (trigger=' + reason + ')...');
    ...
    if (typeof window !== 'undefined' && window.SharkExportEngine && typeof window.SharkExportEngine.cleanup === 'function') {
      try {
        window.SharkExportEngine.cleanup();
      } catch (_) {}
    }
    ...
    if (typeof window !== 'undefined' && typeof window.redrawComposition === 'function') {
      try {
        ...
        console.info('[SharkStudio] ✅ Cache purge finished. Projects & media preserved.');
        return true;
      } finally {
  ...
  return {
    ...
    importAlightMotionXML: importAlightMotionXML,
...
// Compatibility alias: sharktools-adapter.js and legacy tools use FishDatabase
window.FishDatabase = window.SharkDatabase;
