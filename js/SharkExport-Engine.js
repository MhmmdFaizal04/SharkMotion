/**
 * SharkExport-Engine.js
 * SharkTools Studio — Offline Frame-by-Frame Video Export Engine
 *
 ...
 *
 * Public API:
 *   window.SharkExportEngine.export(options) → Promise<void>
...
(function () {
  ...
  function logExport(stage, msg, data) {
    ...
    var prefix = '%c[SharkExport:' + stage + ' @ ' + ts + '] ' + msg;
    if (data !== undefined) {
  ...
  function logExportWarn(stage, msg, data) {
    var ts = (performance.now() / 1000).toFixed(2) + 's';
    var prefix = '%c[SharkExport:' + stage + ' @ ' + ts + '] ⚠️ ' + msg;
    if (data !== undefined) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btn) {
      btn.addEventListener('click', function() {
        if (window.SharkExportEngine && typeof window.SharkExportEngine.cancel === 'function') {
          window.SharkExportEngine.cancel();
    ...
    if (!window.FFmpeg || typeof window.FFmpeg.createFFmpeg !== 'function') {
      await new Promise(function(resolve, reject) {
        ...
        s.onload = function() {
          ...
          else reject(new Error('[SharkExport] FFmpeg script did not register window.FFmpeg'));
        };
        s.onerror = function() { reject(new Error('[SharkExport] Failed to load vendor/ffmpeg/ffmpeg.min.js')); };
  ...
  async function getLayerBlob(layer) {
    ...
    if (mediaId) {
      ...
      if (window.SharkDatabase && typeof window.SharkDatabase.getMedia === 'function') {
        try {
          var dbItem = await window.SharkDatabase.getMedia(mediaId);
          if (dbItem) {
    ...
    try {
      ...
      ffmpeg.setLogger(function(m) {
        if (m && m.message && !m.message.startsWith('frame=')) {
          console.log('%c[SharkExport:FFmpegCore]', 'color:#94a3b8;', m.message);
        }
      });
      ...
      hideProgress();
      alert('[SharkExport] Failed to load FFmpeg WebAssembly. Check vendor files.');
      return;
    }
  ...
  async function doExport(options) {
    ...
    console.log('%c[SharkExport] 🎬 EXPORT STARTED', 'background:#059669;color:#ffffff;font-size:13px;font-weight:bold;padding:4px 8px;border-radius:4px;');
  ...
  window.FishExportEngine = window.SharkExportEngine;

  console.info('[SharkExport-Enggine] Offline deterministic render engine ready.');
})();
