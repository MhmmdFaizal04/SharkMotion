/**
 * SharkMotion Controller & Dispatcher
 * Bridges SharkMotion Panel actions into SharkTools Studio Editor.
...
(function(window) {
  ...
  function applyBeatWarpEffect(customFrames = 8) {
    ...
    const durationSec = Number((frames / fps).toFixed(3)); // 8 frames (punchy beat warp matching SharkMotion)
    const selId = window.selectedLayerId;
    ...
    const fx = (window.SharkEffects && window.SharkEffects.registry && typeof window.SharkEffects.registry.createInstance === 'function')
      ? window.SharkEffects.registry.createInstance('wave-warp')
      : null;
    const waveWarpFx = fx || {
  ...
  function applyBeatFisheyeEffect(scaleFactor = 2) {
    ...
    const adj = (typeof window.addAdjustmentLayer === 'function') ? window.addAdjustmentLayer(durationSec, startSec) : null;
    if (!adj) return JSON.stringify({ error: true, message: 'Failed to create adjustment layer' });
    adj.name = 'Shark Eye Ripple';

    const fx = (window.SharkEffects && window.SharkEffects.registry && typeof window.SharkEffects.registry.createInstance === 'function')
      ? window.SharkEffects.registry.createInstance('warp')
      : null;
    const warpFx = fx || {
    ...
    if (typeof window.syncEffectsRackUI === 'function') window.syncEffectsRackUI();
    if (typeof window.showEffectsRackToast === 'function') window.showEffectsRackToast('Shark Eye Ripple applied');
    return JSON.stringify({ error: false, message: 'Shark Eye Ripple applied', layerId: adj.id });
  }
  ...
  function applyBeatFlashEffect() {
    ...
    const fx = (window.SharkEffects && window.SharkEffects.registry && typeof window.SharkEffects.registry.createInstance === 'function')
      ? window.SharkEffects.registry.createInstance('exposure-gamma')
      : null;
    const expoFx = fx || {
  ...
  function applyBeatBlurEffect(isRightClick) {
    ...
    if (isLensBlur) {
      const fx = (window.SharkEffects && window.SharkEffects.registry && typeof window.SharkEffects.registry.createInstance === 'function')
        ? window.SharkEffects.registry.createInstance('camera-lens-blur')
        : null;
      blurFx = fx || {
      ...
      const fx = (window.SharkEffects && window.SharkEffects.registry && typeof window.SharkEffects.registry.createInstance === 'function')
        ? window.SharkEffects.registry.createInstance('fast-box-blur')
        : null;
      blurFx = fx || {
  ...
  // --- BEAT & PANNING EFFECTS (NULL LAYER RIG MATCHING SHARKMOTION) ---
  function applyBeatNullTool(toolName) {
    const layers = (window.currentProjectState && window.currentProjectState.layers) || [];
    let selId = window.selectedLayerId || (window.selectedLayerIds && window.selectedLayerIds.size === 1 ? Array.from(window.selectedLayerIds)[0] : null);
    let targetLayer = layers.find(l => l.id === selId);

    if (!targetLayer) {
      if (layers.length > 0) {
        // Auto-select top visual or topmost layer if none explicitly selected
        targetLayer = layers.slice().reverse().find(l => l.type !== 'null') || layers[layers.length - 1];
        selId = targetLayer.id;
        if (typeof window.selectTimelineLayer === 'function') {
          window.selectTimelineLayer(selId, false);
        } else {
          window.selectedLayerId = selId;
          if (window.selectedLayerIds) {
            window.selectedLayerIds.clear();
            window.selectedLayerIds.add(selId);
          }
        }
      } else {
    ...
    let allMarkers = Array.from(new Set(rawMarkers.map(m => Number(m.toFixed(4))))).sort((a, b) => a - b);
    let markers = allMarkers.filter(b => b >= (startSec - 0.001) && b <= (endSec + 0.001));

    // Auto-generate rhythmic beat marks if none exist on layer so beat tools work immediately
    const isPanning = toolName.indexOf('PANNING') === 0;
    if (!isPanning && (allMarkers.length === 0 || markers.length === 0)) {
      const step = Math.min(0.5, Math.max(0.25, durationSec / 4));
      const autoMarkers = [];
      for (let t = startSec + step; t < endSec; t += step) {
        autoMarkers.push(Number(t.toFixed(4)));
      }
      if (autoMarkers.length === 0) {
        autoMarkers.push(Number((startSec + durationSec * 0.5).toFixed(4)));
      }
      if (!Array.isArray(targetLayer.markers)) targetLayer.markers = [];
      autoMarkers.forEach(bm => {
        if (!targetLayer.markers.includes(bm)) targetLayer.markers.push(bm);
      });
      if (pState) {
        if (!Array.isArray(pState.beatmarks)) pState.beatmarks = [];
        autoMarkers.forEach(bm => {
          if (!pState.beatmarks.includes(bm)) pState.beatmarks.push(bm);
        });
      }
      if (Array.isArray(window.beatmarks)) {
        autoMarkers.forEach(bm => {
          if (!window.beatmarks.includes(bm)) window.beatmarks.push(bm);
        });
      }
      allMarkers = Array.from(new Set(allMarkers.concat(autoMarkers))).sort((a, b) => a - b);
      markers = allMarkers.filter(b => b >= (startSec - 0.001) && b <= (endSec + 0.001));
      if (typeof window.renderTimelineBeatMarkers === 'function') {
        window.renderTimelineBeatMarkers();
      }
    }
    ...
    // Map toolName to AE SharkMotion Null Name
    let nullName = 'Null';
    ...
    // 1. Create Null Layer matching SharkMotion structure
    const nullLayer = {
    ...
    // 4. Attach Slider Controls & Pure Mathematical Expressions (Matching GitHub SharkMotion)
  ...
  // ======================================================================
  // CENTRAL DISPATCHER: executeSharkTool
  // ======================================================================
  function executeSharkTool(tool, ...args) {
    if (!tool) return;
    if (typeof window !== 'undefined') {
      window._lastDirectSharkToolCall = { key: tool + ':' + JSON.stringify(args), time: Date.now() };
    }
    switch (tool) {
      // Beat Effects
      case 'GHST':
      ...
      // Null Rigs (matching After Effects SharkMotion)
      case 'OSCILLATE':
      case 'SWING':
      case 'Y_BEAT':
      case 'Y_FLIP':
  ...
  // Export to window
  window.executeSharkTool = executeSharkTool;
  ...
  window.executeSharkTool = executeSharkTool;
  window.executeFishTool = executeSharkTool;

  // Cross-frame message listener for postMessage triggers from SharkMotion / FishTools panel iframe
  if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
    window.addEventListener('message', (event) => {
      if (event && event.data && (event.data.type === 'sharktools-run-tool' || event.data.type === 'fishtools-run-tool') && event.data.tool) {
        const args = Array.isArray(event.data.args) ? event.data.args : [];
        executeSharkTool(event.data.tool, ...args);
      }
    });
  }
