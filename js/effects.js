 ...
 * Decoupled effect pipeline for SharkTool Studio.
...
(function(global) {
  ...
  const SharkEffects = {
    registry: SharkEffectsRegistry,

    getParamIds(fx) {
      if (!fx) return [];
      const def = SharkEffectsRegistry.get(fx.type);
    ...
    buildFilter(layer) {
      ...
      if (Array.isArray(layer.effects) && layer.effects.length > 0) {
        ...
        const def = SharkEffectsRegistry.get('brightness-contrast');
    ...
    renderRGBSplit(ctx, el, layer, bounds, fx) {
      const def = SharkEffectsRegistry.get('rgb-split');
    ...
    renderTile(ctx, el, layer, bounds, fx, rgbSplitFx) {
      const def = SharkEffectsRegistry.get('tile');
      if (def && typeof def.render === 'function') {
    ...
    renderLayer(ctx, el, layer, bounds, currentSec) {
      ...
      const renderEffects = effects.filter(f => {
        const def = SharkEffectsRegistry.get(f.type);
    ...
    loadEffectFromXML(xmlText) {
      ...
      try {
        ...
        const existing = SharkEffectsRegistry.get(id);
        const def = {
          id,
          name,
          category,
        ...
        SharkEffectsRegistry.register(def);
        return def;
      } catch (e) {
        console.error('[SharkEffects:XML] Error parsing effect XML:', e);
        return null;
      }
    },
    ...
    renderCardHTML(fx, layer, activeProperty, currentSec) {
      ...
      const controlsHTML = (def.params || []).map(p => {
        ...
        if (type === 'curve') {
          ...
          const curveDef = SharkEffectsRegistry.get('curve');
  ...
  global.SharkEffectsRegistry = SharkEffectsRegistry;
  global.SharkEffects = SharkEffects;
