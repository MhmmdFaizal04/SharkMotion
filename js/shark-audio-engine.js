/**
 * SharkTool Studio - SharkAudioEngine
...
(function(window) {
  ...
  class SharkAudioEngine {
    ...
    _hasUserActivation(e) {
      ...
      if (typeof navigator !== 'undefined' && navigator.userActivation) {
        if (navigator.userActivation.hasBeenActive || navigator.userActivation.isActive) return true;
      }
      return true;
    }
      ...
      activeElementTargets.forEach((info, el) => {
        ...
        if (el.paused) {
          try {
            ...
            // Only seek if difference from target is noticeable (> 80ms)
            if (Math.abs((el.currentTime || 0) - targetTime) > 0.08) {
              try {
            ...
            if (!el._playInProgress) {
              el._playInProgress = true;
              const playPromise = el.play();
              if (playPromise && typeof playPromise.then === 'function') {
                playPromise.then(() => {
                  el._playInProgress = false;
                  if (!isMuted && el.muted) {
                    el.muted = false;
                  }
                  this.setElementGain(el, currentVol);
        ...
        if (!el.seeking) {
          ...
          // Hard seek only on massive divergence (> 1.5s), throttled to once every 1000ms
          if (absDrift > 1.5 && (!el._lastHardSeekTime || (nowMs - el._lastHardSeekTime > 1000))) {
            try {
  ...
  window.SharkAudioEngine = new SharkAudioEngine();
  window.FishAudioEngine = window.SharkAudioEngine;
})(window);
