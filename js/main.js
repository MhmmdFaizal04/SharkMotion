/**
 * SharkMotion Studio - Main JavaScript
 * Handles data fetching from SharkDatabase, New Project creation,
...
function syncWelcomeVersionTags(pkgVersion) {
  ...
  if (!detected) {
    detected = window.OFT_VERSION || '0.1';
  }
  ...
  if (welcomeBadge && !welcomeBadge.dataset.static) {
    welcomeBadge.textContent = 'Alight Motion Web';
  }

  // 3. Dynamically assign Latest badge only to the first version block if changelog exists
...
/**
 * Loads projects from SharkDatabase and populates the "Your Project" list
 */
async function initProjectsFetcher() {
  ...
  async function loadAndRender() {
    let projects = [];
    if (window.SharkDatabase && typeof window.SharkDatabase.getProjects === 'function') {
      try {
        projects = await window.SharkDatabase.getProjects();
      } catch (e) {
        projects = [];
      }
    }
  ...
  window.addEventListener('shark-db-projects-updated', () => {
    loadAndRender();
  });
    ...
    window.ContextMenu.bindTrigger(listContainer, '.project-item', (target) => {
      ...
      return [
        ...
        },
        {
          ...
          action: async () => {
            if (window.SharkDatabase) {
              await window.SharkDatabase.duplicateProject(projectId);
              await loadAndRender();
            }
          }
        },
...
/**
 * Synchronizes project count badges across sidebar and main stage
 */
function updateProjectCountBadges(count) {
  const b1 = document.getElementById('project-count-badge');
  const b2 = document.getElementById('project-count-badge-main');
  const countStr = String(count || 0);
  if (b1) {
    b1.textContent = countStr;
    b1.setAttribute('title', `${countStr} Total Projects`);
  }
  if (b2) {
    b2.textContent = countStr;
    b2.setAttribute('title', `${countStr} Total Projects`);
  }
}
...
function renderProjects(projects, container, countBadge) {
  ...
  updateProjectCountBadges(total);
  ...
  if (!projects || projects.length === 0) {
    container.innerHTML = `
      <div class="projects-empty">
        ...
        <span class="empty-title">No projects yet</span>
        <span class="empty-subtitle">Click "New Project" or choose an aspect ratio above to start creating</span>
        <button type="button" class="btn-create-first" onclick="Modal.open('modal-new-project')">
  ...
  container.innerHTML = projects.map(project => {
    ...
    const ratioClass = `ratio-${aspect.replace(':', '-')}`;
  ...
  if (window.SharkDatabase && typeof window.SharkDatabase.getProjectTotalSize === 'function') {
    (projects || []).forEach(project => {
      if (!project || !project.id) return;
      window.SharkDatabase.getProjectTotalSize(project.id).then(res => {
        ...
        if (project.size !== res.formatted) {
          ...
          if (typeof window.SharkDatabase.updateProjectSize === 'function') {
            window.SharkDatabase.updateProjectSize(project.id, res.formatted, res.bytes);
          }
        }
      }).catch(() => {});
    });
...
/**
 * Quick create project with preselected aspect ratio
 */
function quickCreateWithRatio(ratio) {
  const aspectGrid = document.getElementById('options-aspect-ratio');
  if (aspectGrid) {
    aspectGrid.querySelectorAll('.aspect-ratio-frame').forEach(f => {
      f.classList.toggle('is-selected', f.dataset.val === ratio);
    });
  }
  if (window.Modal) {
    window.Modal.open('modal-new-project');
  }
}
window.quickCreateWithRatio = quickCreateWithRatio;

/**
 * Initializes real-time project searching by title
 */
function initProjectSearch() {
  ...
  const performFilter = () => {
    ...
    const cards = container.querySelectorAll('.project-swipe-container');
    let visibleCount = 0;
    cards.forEach(card => {
      const name = (card.dataset.name || '').toLowerCase();
    ...
    let noMatchEl = container.querySelector('.projects-no-match');
    if (visibleCount === 0 && cards.length > 0) {
      if (!noMatchEl) {
        noMatchEl = document.createElement('div');
        noMatchEl.className = 'projects-no-match';
        noMatchEl.innerHTML = `
          <div class="projects-empty-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" fill="currentColor">
          ...
          <span>No projects match "${escapeHtml(searchInput.value)}"</span>
        `;
        container.appendChild(noMatchEl);
      } else {
        const textSpan = noMatchEl.querySelector('span');
        if (textSpan) textSpan.textContent = `No projects match "${searchInput.value}"`;
      }
    } else if (noMatchEl) {
      noMatchEl.remove();
    }
...
/**
 * Initializes mobile sidebar off-canvas drawer toggle
 */
...
async function openProjectSettingsModal(projectId) {
  ...
  let project = null;
  if (window.SharkDatabase && typeof window.SharkDatabase.getProject === 'function') {
    try {
      project = await window.SharkDatabase.getProject(projectId);
    } catch (e) {
...
async function saveProjectSettingsAction() {
  ...
  if (!projectId || !window.SharkDatabase) return;
  ...
  try {
    const project = await window.SharkDatabase.getProject(projectId);
    if (project) {
      ...
      await window.SharkDatabase.saveProject(project);
...
async function confirmDeleteProjectAction() {
  ...
  if (listContainer && projectId) {
    ...
    updateProjectCountBadges(remaining);
    if (remaining === 0) {
...
async function exportProjectAction(projectId, projectName) {
  ...
  if (window.SharkDatabase && typeof window.SharkDatabase.exportProjectToOFTS === 'function') {
    try {
      await window.SharkDatabase.exportProjectToOFTS(projectId);
...
async function createNewProjectAction() {
  ...
  try {
    if (window.SharkDatabase && typeof window.SharkDatabase.createProject === 'function') {
      await window.SharkDatabase.createProject({
        id: projectId,
        name: name,
    ...
    console.warn('SharkDatabase createProject error, using fallback:', err);
  }
...
/**
 * Handles imported .ofts or .xml project files
 */
async function handleImportedFiles(files, dropzone, statusEl) {
  if (!files || files.length === 0) return;
  const file = files[0];
  const isOFTS = file && file.name.toLowerCase().endsWith('.ofts');
  const isXML = file && file.name.toLowerCase().endsWith('.xml');

  if (!isOFTS && !isXML) {
    alert('Only .ofts and Alight Motion .xml files are supported');
    return;
  }
  ...
  if (isOFTS) {
    if (window.SharkDatabase && typeof window.SharkDatabase.importOFTSPackage === 'function') {
      try {
        importedProject = await window.SharkDatabase.importOFTSPackage(file);
      } catch (e) {
        console.warn('OFTS package import error:', e);
        alert('Failed to import .ofts file: ' + (e.message || e));
      }
    }
  } else if (isXML) {
    if (window.SharkDatabase && typeof window.SharkDatabase.importAlightMotionXML === 'function') {
      try {
        importedProject = await window.SharkDatabase.importAlightMotionXML(file);
      } catch (e) {
        console.warn('Alight Motion XML import error:', e);
        alert('Failed to import Alight Motion .xml file: ' + (e.message || e));
      }
    }
  }
