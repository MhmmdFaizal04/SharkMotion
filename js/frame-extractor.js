/**
 * SharkTool Studio - VideoFrameExtractor
...
(function(window) {
  ...
  const DB_NAME = 'SharkFrameCacheDB';
  const DB_VERSION = 4;
  ...
  class VideoFrameExtractor {
    ...
    async extractLayerRange(layer) {
      ...
      // Ensure layer.mediaId and fresh layer.dataUrl from SharkDatabase before resolving sourceKey
      if (window.SharkDatabase && window.currentProjectState && window.currentProjectState.id) {
        try {
          const medias = await window.SharkDatabase.getProjectMedia(window.currentProjectState.id);
